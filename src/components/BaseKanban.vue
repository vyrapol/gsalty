<template>
  <section class="grid grid-cols-3">
    <div class="bg-red-500 kanban-card">
      <div class="w-28 h-[21px] justify-start items-center gap-2 inline-flex">
        ...
        <div class="text-zinc-900 text-[16px] font-medium capitalize">({{ applied }})</div>
      </div>
      <div class="kanban-item-container">
        <div v-for="i in applied" ref="target" class="kanban-item" @contextmenu.prevent="onContext($event, i, 'applied', 'test')">
          ...
          <section class="relative mb-4">
            <div>
              <span class="text-gray-500 text-[14px] font-normal capitalize"
                >Tags:</span
              ><span class="text-zinc-900 text-[14px] font-normal capitalize">
                Sok Sanrathana</span
              >
            </div>
          ...
          <section class="flex gap-2">
            ...
            >
              <div class="w-4 h-4 relative">
                <div class="w-4 h-4 left-0 top-0 absolute bg-zinc-300"></div>
              </div>
    ...
    <div class="bg-green-500 kanban-card">
      ...
      <div class="kanban-item-container">
        <div v-for="i in test" ref="target" class="kanban-item" @contextmenu.prevent="onContext($event, i, 'test', 'list')">
          <section class="relative mb-6">
            <div class="w-[184px] h-[45px] relative">
              <div
                class="left-[57px] top-[1px] absolute text-slate-800 text-[16px] font-normal capitalize"
  ...
  <div class="context-menu" ref="ctx">
    <div
      class="w-[46px] h-16 p-2 bg-purple-50 rounded-lg shadow flex-col justify-start items-start gap-2 inline-flex"
    >
      <div class="text-black text-[12px] font-normal capitalize" @click="move">move</div>
      <div class="h-px w-full bg-gray-200 before:content-[''] before:block before:h-px"></div>
      <div class="text-zinc-900 text-[12px] font-normal capitalize">fails</div>
    </div>
  </div>
</template>
...
const onContext = (e, i, fromBoard, toBoard) => {
...
const move = () => {
  if (from.value === 'applied') { applied.value -= 1; test.value += 1 }
}
...
.kanban-card {
  @apply flex flex-col w-full h-full min-h-[700px] text-left p-2 gap-4;
}
...
.kanban-item {
  @apply bg-white rounded-lg shadow p-4 relative;
}
